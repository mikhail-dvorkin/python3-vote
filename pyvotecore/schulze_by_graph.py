...
class SchulzeMethodByGraph(SchulzeMethod):
    ...
    def standardize_ballots(self, ballots, ballot_notation):
        self.ballots = []
        self.candidates = set([edge[0] for edge, weight in self.edges.items()]) | set([edge[1] for edge, weight in self.edges.items()]) #iteritems x2

    def ballots_into_graph(self, candidates, ballots):
        graph = digraph()
        graph.add_nodes(candidates)
        for edge in self.edges.items(): # iter
...
class SchulzeNPRByGraph(AbstractOrderingVotingSystem, SchulzeHelper):
    ...
    def __init__(self, edges, winner_threshold=None, tie_breaker=None, ballot_notation=None):
        self.edges = edges
        self.candidates = set([edge[0] for edge, weight in edges.items()]) | set([edge[1] for edge, weight in edges.items()]) # iter x2
    ...
    def ballots_without_candidate(self, ballots, candidate):
        self.edges = dict([(edge, weight) for edge, weight in self.edges.items() if edge[0] != candidate and edge[1] != candidate]) # iter
        return self.edges
